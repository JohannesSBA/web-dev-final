...
export default async function Dashboard() {
  ...
  for (const chat of chats) {
    if (!chat.groupId || !chat.userId) {
      continue;
    }

    if (!participantMap.has(chat.groupId)) {
      participantMap.set(chat.groupId, new Set<string>());
    }

    participantMap.get(chat.groupId)!.add(chat.userId);
  }

  return (
    <div className="relative min-h-screen w-full bg-linear-to-b from-gray-950 via-gray-950 to-black text-white">
      <div className="absolute top-4 right-4">
        {process.env.DEV_BYPASS_AUTH === "true" ? null : <SignOut />}
      </div>
      ...
      <main className="mx-auto flex min-h-screen max-w-4xl flex-col px-4 py-16 sm:px-6">
        ...
        <section aria-labelledby="groups-heading" className="flex-1">
          ...
          <ul className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {groups.map((group) => {
              const ownerName = group.ownerName?.trim() || "Unknown";
              const groupId = group._id.toString();
              const participantCount = participantMap.get(groupId)?.size ?? 0;
              ...
              return (
                <li key={groupId}>
                  ...
                  >
                    <p className="truncate text-lg font-semibold text-white group-hover:text-blue-100">
